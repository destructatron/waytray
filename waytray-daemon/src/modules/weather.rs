...
impl WeatherModule {
    ...
    fn create_module_item(&self, data: &WttrResponse) -> Option<ModuleItem> {
        let condition = data.current_condition.first()?;
        let area = data.nearest_area.first();

        let use_fahrenheit = self.config.units.to_lowercase() == "fahrenheit";
...
impl Module for WeatherModule {
    ...
    async fn start(&self, ctx: Arc<ModuleContext>) {
        ...
        tracing::info!("Weather module starting, location: {}",
            if self.config.location.is_empty() { "auto-detect" } else { &self.config.location });

        // Fetch initial weather
        if let Some(data) = self.fetch_weather().await {
            if let Some(item) = self.create_module_item(&data) {
